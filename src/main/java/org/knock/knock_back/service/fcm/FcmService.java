					...
					switch (i) {
						case 1 -> {
							...
							for (String id : idList) {
								...
								if (movie.getOpeningTime() >= todayMillis
									&& movie.getOpeningTime() <= notifyTargetMillis) {
									...
									for (String token : user.getDeviceToken()) {
										sendMessageByToken(token,
											movie.getMovieNm() + " 개봉 D-" + (remainTime == 0 ? "DAY" : remainTime)
												+ "!", "기다리셨던 그 컨텐츠 지금 확인해보세요!");
									}
								}
							}
						}
						...
						case 2 -> {
							...
							for (String id : idList) {
								...
								if (koficEpochMillis >= todayMillis && koficEpochMillis <= notifyTargetMillis) {
									...
									for (String token : user.getDeviceToken()) {
										sendMessageByToken(token,
											kofisIndex.getName() + " 개봉 D-" + (remainTime == 0 ? "DAY" : remainTime)
												+ "!", "기다리셨던 그 컨텐츠 지금 확인해보세요!");
									}
								}
							}
