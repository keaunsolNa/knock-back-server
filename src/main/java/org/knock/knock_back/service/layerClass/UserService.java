...
public class UserService {
		...
		try {
			...
			for (String id : list) {
				...
				if (kopisRepository.findById(id).isPresent())
					kopis = kopisRepository.findById(id).get();
				if (kopis.getCategoryLevelTwo().getNm().equals(value))
					returnValue.add(id);
	...
	public Boolean saveDeviceToken(String targetToken) {
		...
		try {
			...
			if (null == user.getDeviceToken() || user.getDeviceToken().isEmpty())
				user.setDeviceToken(new ArrayList<>());

			if (user.getDeviceToken().contains(targetToken))
				return true;
	...
	private Integer CategoryLevelOneUpdate(CategoryLevelOne target, String targetId, String userId, boolean flag) {
		...
		switch (target) {
			case CategoryLevelOne.MOVIE -> {
				...
				if (flag)
					movieIndex.getFavorites().add(userId);
				else
					movieIndex.getFavorites().remove(userId);
			...
			case CategoryLevelOne.PERFORMING_ARTS -> {
				...
				if (flag)
					performingIndex.getFavorites().add(userId);
				else
					performingIndex.getFavorites().remove(userId);
